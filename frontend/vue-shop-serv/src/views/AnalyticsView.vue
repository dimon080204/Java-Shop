...
// Фильтры (текущий месяц по умолчанию)
...
const fetchAnalytics = async () => {
  try {
    ...
    // Формируем корректные даты для Spring Boot
    ...
    // СМОТРИМ СЮДА В КОНСОЛИ F12:
    console.log("ABC Data from server:", abcRes.data);
    console.log("KPI Data from server:", kpiRes.data);
  ...
  <div class="page-wrapper">
    <div class="container">
      ...
      <div v-else class="analytics-grid">
        <!-- Таблица ABC (Данные приходят готовыми из AnalyticsDTO) -->
        ...
        <!-- Таблица KPI (Данные приходят готовыми из StaffKPIDTO) -->
        <div class="analytics-card">
          <div class="card-header">
...
<script>
// Добавим пару хелперов для визуализации KPI прямо в компоненте
export default {
  methods: {
    getKpiColor(score) {
      if (score >= 1.2) return '#27ae60'; // Зеленый (лучше среднего)
      if (score >= 0.8) return '#3498db'; // Голубой (норма)
      return '#e74c3c'; // Красный (ниже нормы)
    },
    getKpiClass(score) {
...
/* Стили оставляем те же, добавляем пару штрихов */
...
/* Остальные стили из предыдущего сообщения копируем сюда без изменений */
